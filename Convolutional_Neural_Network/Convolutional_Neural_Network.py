...
class Convolutional_Neural_Network():
    ...
    def train(self, x, T, learning_rate):
        ...
        print "Conv_layer_roop"
        for conv_layer_roop in xrange(self.number_of_convpool_layers):
            if conv_layer_roop == 0:
                conv_x = x
            
            conv_x = self.conv_pool_layers[conv_layer_roop].forward(conv_x)

            if ( conv_layer_roop == (self.number_of_convpool_layers - 1) ):
                self.conved = conv_x
        
        print "Hidden_layer_roop"
        for hidden_layer_roop in xrange(self.number_of_hidden_layers):
            if(hidden_layer_roop == 0):
                hidden_x = self.flatten(self.conved)
            
            hidden_x = self.hidden_layers[hidden_layer_roop].forward(hidden_x)

            if( hidden_layer_roop == (self.number_of_hidden_layers - 1) ):
        ...
        for hidden_layer_roop in reversed(xrange(0, self.number_of_hidden_layers)):

            if(hidden_layer_roop == self.number_of_hidden_layers - 1):
                print "layer%d" %(hidden_layer_roop + 1)
                self.hidden_layers[hidden_layer_roop].backward(self.logistic_layer, learning_rate)
        
            else:
                print "layer%d" %(hidden_layer_roop + 1)
                self.hidden_layers[hidden_layer_roop].backward(self.hidden_layers[hidden_layer_roop + 1], learning_rate)

            if(hidden_layer_roop == 0):
                self.back_hiddened = self.hidden_layers[0].delta

        print "Back the conv_layer"
        for conv_layer_roop in reversed(xrange(0, self.number_of_convpool_layers)):
            
            if ( conv_layer_roop == (self.number_of_convpool_layers - 1) ):
                unflattened = self.unflatten(self.back_hiddened)
                print "layer%d" %(conv_layer_roop + 1)
                self.conv_pool_layers[conv_layer_roop].backward(unflattened, learning_rate)

            else:
                print "layer%d" %(conv_layer_roop + 1)
                self.conv_pool_layers[conv_layer_roop].backward(self.conv_pool_layers[conv_layer_roop + 1].dconved_delta, learning_rate)
    ...
    def pretest(self):
        ...
        for layer_roop in xrange(self.number_of_hidden_layers):

            if(layer_roop == 0):
                pretest_In = self.flattened_size

            else:
                pretest_In = self.hidden_layer_sizes[layer_roop - 1]
        
            if( layer_roop == (self.number_of_hidden_layers - 1) ):
                pretest_Out = self.number_of_class

            else:
                pretest_Out = self.hidden_layer_sizes[layer_roop + 1]
        
            for i in xrange(pretest_Out):
                for j in xrange(pretest_In):
                    self.hidden_layer[layer_roop].W[i][j] *= (1 - self.drop_out_possibility)
    ...
    def predict(self, Input):
        ...
        for conv_layer_roop in xrange(self.number_of_convpool_layers):
            ...
            conv = self.conv_pool_layers[conv_layer_roop].forward(conv)
        ...
        for hidden_layer_roop in xrange(self.number_of_hidden_layers):
            ...
            hidden = self.hidden_layers[hidden_layer_roop].forward(hidden)
